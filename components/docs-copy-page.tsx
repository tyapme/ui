...
import { Button, buttonVariants } from "@/styles/base/ui/button"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const menuItems: Record<string, MenuItemData> = {
  ...
  scira: {
    href: (url) => getPromptUrl("https://scira.ai/", url),
    target: "_blank",
    rel: "noopener noreferrer",
    icon: (
      <svg
...
const triggerClassName = cn(
  ...
  "size-8 shadow-none md:size-7 md:text-[0.8rem]"
)
    ...
    <Popover>
      <div className="group/buttons relative flex rounded-lg bg-secondary *:[[data-slot=button]]:focus-visible:relative *:[[data-slot=button]]:focus-visible:z-10">
        ...
        <Separator
          ...
          className="h-6 bg-border data-vertical:self-center sm:h-5"
        />
        <DropdownMenu>
          <DropdownMenuTrigger
        ...
        <PopoverTrigger className={cn(triggerClassName, "flex sm:hidden")}>
          <IconChevronDown className="rotate-180 sm:rotate-0" />
        </PopoverTrigger>
        <PopoverContent
        ...
        >
          {Object.entries(menuItems).map(([key, item]) => (
            <Button
              ...
              render={
                <a href={item.href(url)} target={item.target} rel={item.rel} />
              }
            >
              {item.icon}
              {item.label}
