---
title: 2026年1月 - インラインスタート・エンドスタイル
description: Base UI コンポーネントのスタイルを更新し、inline-start と inline-end の side 値をサポート。
date: 2026-01-23
---

Base UI コンポーネントのスタイルを更新し、`inline-start` および `inline-end` の side 値をサポートしました。以下のコンポーネントがこれらの値に対応しています：
...
論理的な side 値を扱うための新しい Tailwind クラスを追加しました：
...
以下のプロンプトを LLM に渡すと、コンポーネントを自動更新できます：
