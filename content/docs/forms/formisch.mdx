...
Here's a basic example of a form using the `<Field />` component from Formisch and the TYAP.ME UI `<Field />` component.
...
<Callout icon={<InfoIcon />}>
  **Note:** Formisch ships its own `Field` component. To avoid a name clash with
  the TYAP.ME UI `Field`, the examples below import the Formisch one as
  `FormischField` and keep the TYAP.ME UI `Field` under its original name. In your
...
We can now build the form using the `<Field />` component from Formisch and the TYAP.ME UI `<Field />` component.
