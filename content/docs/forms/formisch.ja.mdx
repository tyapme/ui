...
<Callout icon={<InfoIcon />}>
  **注意:** Formisch は独自の `Field` コンポーネントを持っています。Ketyap UI の `Field` と名前が衝突するため、下記の例では Formisch 側を `FormischField` としてインポートし、Ketyap UI の `Field` は元の名前のまま使用しています。自分のコードではどちら側にもエイリアスを設定できます——一賫していればOKです。
</Callout>
