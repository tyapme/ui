...
## Project-specific: Where to write component code

When working on a component in `registry/ui/`, apply the **component-edit** skill rules:
- Write all changes (layout, spacing, animation, variants) directly into the component's `.tsx` file
- Do NOT add new rules to `style-maia.css` or other CSS token files
- Preserve `"use client"` and other existing file structure exactly as found
