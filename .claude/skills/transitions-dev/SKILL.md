...
# Project-specific: Where to write the code

When applying a transition to a component in `registry/ui/`, write the CSS **directly in the component's `.tsx` file** — not in `style-maia.css`. Use Tailwind utilities in `cva()`/`cn()`, or a `<style precedence href>` tag for complex selectors (`@starting-style`, etc.). Preserve `"use client"` and file structure as-is.
