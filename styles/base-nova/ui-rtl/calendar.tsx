...
function CalendarCellIndicator({
  ...
  return (
    <span
      className={cn(
        "absolute start-1/2 bottom-1 size-1 -translate-x-1/2 rounded-full bg-current rtl:translate-x-1/2",
        className
      )}
      {...props}
    />
        ...
        return (
          <div
            className={cn(
              ...
              isSelectionStart &&
                !isSelectionEnd &&
                "rounded-(--cell-radius) rounded-e-none",
              // Range end: square left edge (only when start ≠ end)
              isSelectionEnd &&
                !isSelectionStart &&
                "rounded-(--cell-radius) rounded-s-none",
              // Unavailable
