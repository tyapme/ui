...
import { buttonVariants } from "@/styles/base/ui-rtl/button"
...
function CalendarCellIndicator({
  ...
  return (
    <span
      className={cn(
        "absolute start-1/2 bottom-0.5 size-[3px] -translate-x-1/2 rounded-full bg-current rtl:translate-x-1/2",
        className
      )}
      {...props}
    />
          ...
          >
            ...
            {isTodayDate && (
              <span
                aria-hidden
                className={cn(
                  "absolute start-1/2 bottom-[3px] size-[3px] -translate-x-1/2 rounded-full rtl:translate-x-1/2",
...
function CalendarRoot<T extends DateValue = DateValue>({
  ...
  return (
    ...
    >
      <AriaCalendar
        className={cn(
          "p-3 [--cell-radius:var(--radius-md,calc(var(--radius)-2px))] [--cell-size:--spacing(8)]",
          "bg-background p-3",
